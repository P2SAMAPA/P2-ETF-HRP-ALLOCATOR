...
class HRPAllocator:
    ...
    def __init__(self, linkage_method: str = 'ward', return_metric: str = 'mean_return', risk_free_rate: float = 0.0):
    ...
    def _leaf_weights(self, cov: pd.DataFrame, returns: pd.DataFrame) -> np.ndarray:
        ...
        if self.return_metric == 'inverse_variance':
            ...
            # Shift to positive if any negative (so weights are positive)
